    ...
    for _, v in ipairs( content_transformer.transform( data ) ) do
      popup.add_line( v.type, function( type, frame, lines )
        ...
        elseif type == "roll" then
          -- Display roll_type with optional spec annotation for MS rolls
          local roll_type_text = m.roll_type_color( v.roll_type, m.roll_type_abbrev( v.roll_type ) )
          if v.spec_annotation then
            roll_type_text = roll_type_text .. " " .. m.colors.grey( "(" .. v.spec_annotation .. ")" )
          end
          frame.roll_type:SetText( roll_type_text )
