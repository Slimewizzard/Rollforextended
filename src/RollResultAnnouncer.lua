...
function M.new( chat, roll_controller, softres, config )
  ...
  local announce_winner = function( winners, top_roll )
    ...
    -- Build roll type string with optional spec annotation for MS/OS rolls
    local roll_type_str = ""
    if roll_type == RT.MainSpec then
      -- Check if RollForSpecs is installed and get main spec for winner
      if RollForSpecs and RollForSpecs.spec_registry then
        local entry = RollForSpecs.spec_registry.get( winners[ 1 ].name )
        if entry then
          local spec_full = RollForSpecs.SpecData.GetFullSpecName( entry.class, entry.main_spec or entry.spec )
          if spec_full then
            roll_type_str = string.format( " (%s)", spec_full )
          end
        end
      end
    elseif roll_type == RT.OffSpec then
      -- Check if RollForSpecs is installed and get off spec for winner
      if RollForSpecs and RollForSpecs.spec_registry then
        local entry = RollForSpecs.spec_registry.get( winners[ 1 ].name )
        if entry and entry.off_spec then
          local spec_full = RollForSpecs.SpecData.GetFullSpecName( entry.class, entry.off_spec )
          if spec_full then
            roll_type_str = string.format( " (%s)", spec_full )
          else
            roll_type_str = " (os)"
        ...
        roll_type_str = string.format( " (%s)", m.roll_type_abbrev_chat( roll_type ) )
      end
    else
      roll_type_str = string.format( " (%s)", m.roll_type_abbrev_chat( roll_type ) )
    end
