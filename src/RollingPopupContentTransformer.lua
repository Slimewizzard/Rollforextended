...
function M.new( config )
  ...
  local function add_rolls( result, rolls )
    ...
    for i = 1, getn( rolls ) do
      ...
      -- Check if RollForSpecs is installed and get spec for MS/OS rolls
      local spec_annotation = nil
      if RollForSpecs and RollForSpecs.spec_registry then
        local entry = RollForSpecs.spec_registry.get( roll.player_name )
        if entry then
          if roll.roll_type == RT.MainSpec then
            spec_annotation = RollForSpecs.SpecData.GetFullSpecName( entry.class, entry.main_spec or entry.spec )
          elseif roll.roll_type == RT.OffSpec then
            spec_annotation = RollForSpecs.SpecData.GetFullSpecName( entry.class, entry.off_spec )
          end
        end
      end
      ...
      table.insert( result, {
        ...
        roll = roll.roll,
        spec_annotation = spec_annotation,
